...
func (f *FolderService) ListUserFolders(ctx context.Context, userID int, username string, queryParams folder.Query) (*model.FoldersSummary, error) {
	return f.repo.ListFolders(ctx, userID, username, queryParams)
}

func (f *FolderService) GetFolder(ctx context.Context, userID int, username, slug string) (*model.Folder, error) {
	return f.repo.GetFolder(ctx, userID, username, slug)
}

func (f *FolderService) CreateFolder(ctx context.Context, userID int, title string) (string, error) {
	return f.repo.CreateFolder(ctx, userID, title)
}

func (f *FolderService) UpdateFolder(ctx context.Context, userID int, slug string, title string) (string, error) {
	return f.repo.UpdateFolder(ctx, userID, slug, title)
}

func (f *FolderService) DeleteFolder(ctx context.Context, userID int, username, slug string) error {
